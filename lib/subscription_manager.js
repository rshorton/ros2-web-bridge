...
class SubscriptionManager {
  ...
  // added transient local support
  createSubscription(messageType, topicName, use_transient_local, bridgeId, callback) {
    let handle = this._subscripions.get(topicName);

    if (!handle) {
      //scott
      let my_qos = QoS.profileDefault;
      try {
        if (use_transient_local) {
          console.log("Subscribing with QoS RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL");
          my_qos = new QoS(QoS.HistoryPolicy.RMW_QOS_POLICY_HISTORY_KEEP_LAST, 1, QoS.ReliabilityPolicy.RMW_QOS_POLICY_RELIABILITY_RELIABLE,
                           QoS.DurabilityPolicy.RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL, false);
        }
      } catch (err) {
        console.log("err= " + err.message);
      }
      let subscription = this._node.createSubscription(messageType, topicName, {enableTypedArray: false, qos: my_qos}, (message) => {
